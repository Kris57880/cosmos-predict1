...
from cosmos_predict1.autoregressive.inference.world_generation_pipeline import ARVideoCompressPipeline
...
def parse_args():
    ...
    parser.add_argument("--buffer_frames", type=int, default=9, help="Number of buffer frames for video compression")
    parser.add_argument("--disable_entropy_coding", action="store_true", help="Disable entropy coding for compression")
...
def main(args):
    ...
    # Initialize video compression pipeline
    log.info("Initiailizing the compression pipeline...")
    pipeline = ARVideoCompressPipeline(
    ...
    input_videos = load_vision_input(
        ...
        data_resolution=[VIDEO_HEIGHT, VIDEO_WIDTH], 
        num_input_frames=NUM_TOTAL_FRAMES,
    )
    for idx, input_filename in enumerate(input_videos):
        inp_vid = input_videos[input_filename]
        # Run compression pipeline
        log.info(f"Run compression for: {input_filename}")
        out_vid, bpp = pipeline.compress(
            inp_vid=inp_vid,
            buffer_frames=args.buffer_frames, 
            sampling_config=sampling_config,
            seed=args.seed,
            log_dir=args.video_save_folder,
            entropy_coding=not args.disable_entropy_coding,
        )
        if out_vid is None:
            log.critical("Guardrail blocked compression.")
            continue

        # Save each frame as png to a folder
        if args.input_image_or_video_path:
            frame_dir = os.path.join(args.video_save_folder, f"{args.video_save_name}_frames")
        else:
            frame_dir = os.path.join(args.video_save_folder, f"{idx}_frames")
        os.makedirs(frame_dir, exist_ok=True)
        for i, frame in enumerate(out_vid):
            # frame: (H, W, C), float32 or uint8
            # 若 frame 為 float, 轉成 uint8
            if frame.dtype != 'uint8':
                frame = (np.clip(frame, 0, 1) * 255).astype(np.uint8)
            import imageio
            imageio.imwrite(os.path.join(frame_dir, f"frame_{i+1}.png"), frame)
        log.info(f"Saved {len(out_vid)} frames to {frame_dir}")
        # Save bpp result
        bpp_path = os.path.join(args.video_save_folder, f"{args.video_save_name}_bpp.txt")
        with open(bpp_path, "w") as f:
            f.write(f"bpp: {bpp}\n")
        log.info(f"Saved bpp to {bpp_path}")
